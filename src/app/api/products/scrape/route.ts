...
// Fetch via ScraperAPI (fallback for blocked sites)
async function fetchWithScraperAPI(url: string): Promise<string | null> {
  if (!SCRAPER_API_KEY) {
    console.log("[SCRAPER] No ScraperAPI key configured");
    return null;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!html) {
      console.log("[SCRAPER] Direct fetch failed, trying ScraperAPI...");
    ...
    console.log(`[SCRAPER] Successfully fetched via ${usedScraperAPI ? "ScraperAPI" : "direct"}`);
